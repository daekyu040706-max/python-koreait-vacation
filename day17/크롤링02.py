...
# 조건에 맞는 모든 태그를 가져와보자!(리스트) - find_all()
# -> [tag1, tag2...]
all_quote_tags = html.find_all("div", class_="quote")
# 10이면 다 가져온것, 14, 16 -> 조건을 좀 더 엄밀하게
...
for idx, quote_tag in enumerate(all_quote_tags):
    ...
    # 저자url을 가져와보자! - 태그에 숨겨져있는 정보
    #<a href="상대경로 주소">
    author_link_tag = quote_tag.find("a") # 처음발견한 a태그
    link_url = author_link_tag.get("href") # href속성 값을 가져와
    # 상대경로를 절대경로로 변환
    link_url = url + link_url
    print(f"{idx + 1}: {author}({link_url}) - {text}")

    # 도전!) 태그들의 텍스트들을 추출해서 아래 tags에 append해주세요
    tags = []
    # find_all(), for문 사용
    # 1. a태그이면서 clas이름이 "tag"인것들 다 찾기
    a_tags = quote_tag.find_all("a", class_="tag")
    # 2. for문으로 a태그들의 텍스트만 추출하기
    for a_tag in a_tags:
        tag_text = a_tag.text
    # 3. 추출해서 tags리스트에 append하기
...
# select 방식
# CSS 선택자 방식 -> 브라우저가 태그위치를 인식하는 방식

# .어쩌고 -> 어쩌고 클래스인 태그들 다 가져와
# 어쩌고 -> 어쩌고 태그들 다 가져와
# 어쩌고.저쩌고 -> 어쩌고태그이면서 저쩌고 클래스 태그들
# #어쩌고 -> 어쩌고 id인 태그들
